...
<xsl:stylesheet version="1.0"
  ...
  <!-- Since these elements use the frmwrk name space, they will not be executed when the XSLT is applied to the user config file -->
	<frmwrk:uiconfig>
		...
		<frmwrk:compositeNode xPath="/uiConfig/commandBars/commandBarData[@guid='2cfa7035-cbef-4276-b686-adb4f77c80d6']"/>
		<frmwrk:compositeNode xPath="/uiConfig/commandBars/commandBarData[@guid='c2b44f69-6dec-444e-a37e-5dbf7ff43dae']"/>
		<frmwrk:compositeNode xPath="/uiConfig/views/viewTemplate[@guid='785c273f-c462-4f02-9ee4-c354018e0878']"/>
  ...
  <xsl:template match="node()|@*">
	...
	<xsl:template match="commandBarData[@guid='2cfa7035-cbef-4276-b686-adb4f77c80d6']/menu/item[@guidRef='785c273f-c462-4f02-9ee4-c354018e0878']">
		<xsl:copy>
			<xsl:apply-templates select="@*|node()"/>
		</xsl:copy>
		<!-- Make sure we don't read the menu item it it already exists -->
		<xsl:if test="not(./item[@guidRef='6946e359-c240-47c5-888a-e3fc69608d6d'])">
			<item guidRef="6946e359-c240-47c5-888a-e3fc69608d6d"/>
		</xsl:if>
	</xsl:template>
  ...
  <xsl:template match="commandBarData[@guid='c2b44f69-6dec-444e-a37e-5dbf7ff43dae']/toolbar">
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
      <!-- Make sure we don't read the menu item it it already exists -->
      <xsl:if test="not(./item[@guidRef='3cc4d624-7013-428b-9b1b-2245e9602ce8'])">
				<item guidRef="3cc4d624-7013-428b-9b1b-2245e9602ce8"/>
			</xsl:if>
    </xsl:copy>
  </xsl:template>
