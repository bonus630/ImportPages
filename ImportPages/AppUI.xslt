  ...
  <xsl:template match="uiConfig/items">
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
		...
		<!-- Creates a button to add at a menu, menu item needs works with DataSource-->
		<itemData guid="6946e359-c240-47c5-888a-e3fc69608d6d"
				   type="button"
				   onInvoke="*Bind(DataSource=ImportPagesDS;Path=MenuItemCommand)"
				   caption="*Bind(DataSource=ImportPagesDS;Path=Caption)"
				   icon="*Bind(DataSource=ImportPagesDS;Path=Icon)"
				   enable="true"/>
		
		<!--Creates a item to add a Warpper, responsible to initialize the DataSource to be consumimed in menu item above -->
      <itemData guid="3cc4d624-7013-428b-9b1b-2245e9602ce8"
                type="wpfhost"
                hostedType="Addons\ImportPages\ImportPages.CorelAddon,ImportPages.ControlUI"
                enable="true"/>
    ...
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
        ...
        <container>
          ...
          <item dock="fill" margin="0,0,0,0" guidRef="3cc4d624-7013-428b-9b1b-2245e9602ce8"/>
        </container>
